    ...
    async def record_cache_command(
        ...
        try:
            ...
            if denied:
                ...
                return
            if flag is None:
                state = "on" if self._plugin._cache_enabled else "off"
                yield event.plain_result(
                    f"缓存状态：{state}，容量：每会话 {self._plugin._cache_limit} 条"
                )
                return
            ...
            if val == "status":
                ...
                whitelist_info = f"群白名单：{', '.join(self._plugin._group_whitelist) if self._plugin._group_whitelist else '无（记录所有群）'}"
                private_info = f"私聊记录：{'开启' if self._plugin._record_private_chats else '关闭'}"
                sanitize_info = f"数据脱敏：{'开启' if self._plugin._enable_sanitization else '关闭'}"
                export_info = f"导出权限：{'仅管理员' if self._plugin._export_admin_only else '所有人'}"
                yield event.plain_result(
                    f"状态：{'开启' if self._plugin._cache_enabled else '关闭'}\n"
                    f"容量上限：{self._plugin._cache_limit}\n"
                ...
                return
            if val in ("on", "true", "1"):
                self._plugin._cache_enabled = True
                await self._plugin._put_kv_value("cache_enabled", True)
                yield event.plain_result("已开启缓存写入。")
            elif val in ("off", "false", "0"):
                self._plugin._cache_enabled = False
                await self._plugin._put_kv_value("cache_enabled", False)
    ...
    async def record_export_command(
        ...
        try:
            ...
            if flag is None or str(flag).strip().lower() == "status":
                state = "on" if self._plugin._export_admin_only else "off"
                yield event.plain_result(f"导出权限状态：{state}")
                return
            val = str(flag).strip().lower()
            if val in ("on", "true", "1"):
                self._plugin._export_admin_only = True
                await self._plugin._put_kv_value("export_admin_only", True)
                yield event.plain_result("已开启导出管理员限制。")
            elif val in ("off", "false", "0"):
                self._plugin._export_admin_only = False
                await self._plugin._put_kv_value("export_admin_only", False)
    ...
    async def record_admin_command(
        ...
        try:
            ...
            if denied:
                ...
                return
            if flag is None or str(flag).strip().lower() == "status":
                state = "on" if self._plugin._admin_only else "off"
                yield event.plain_result(f"全局管理员权限状态：{state}")
                return
            val = str(flag).strip().lower()
            if val in ("on", "true", "1"):
                self._plugin._admin_only = True
                await self._plugin._put_kv_value("admin_only", True)
                yield event.plain_result("已开启全局管理员权限限制。")
            elif val in ("off", "false", "0"):
                self._plugin._admin_only = False
                await self._plugin._put_kv_value("admin_only", False)
    ...
    async def record_limit_command(
        ...
        try:
            ...
            if n is None:
                ...
                export_info = f"导出权限：{'仅管理员' if self._plugin._export_admin_only else '所有人'}"
                yield event.plain_result(
                    f"当前容量上限：每会话 {self._plugin._cache_limit} 条（可设置范围 1~{self._plugin.MAX_CACHE_LIMIT}）\n"
                    f"当前会话（{name}）：行数={session_lines}，线程数={threads_cnt}\n"
                    f"命中：{hit}，未命中：{miss}，命中率：{hit_rate:.1f}%\n"
                    f"总会话键数：{total_sessions}\n"
                    f"{whitelist_info}\n"
            ...
            async with self._plugin._write_lock:
                self._plugin._reconfigure_cache_limit(int(n))

            await self._plugin._put_kv_value("cache_limit", int(self._plugin._cache_limit))

            yield event.plain_result(
                "已将容量上限设为："
                f"每会话 {self._plugin._cache_limit} 条，现有会话已按新上限裁剪"
            )
        except (ValueError, TypeError) as exc:
            logger.warning(
    ...
    async def record_private_command(
        ...
        try:
            ...
            if denied:
                ...
                return
            if flag is None or str(flag).strip().lower() == "status":
                state = "on" if self._plugin._record_private_chats else "off"
                yield event.plain_result(f"私聊记录状态：{state}")
                return
            val = str(flag).strip().lower()
            if val in ("on", "true", "1"):
                self._plugin._record_private_chats = True
                await self._plugin._put_kv_value("record_private_chats", True)
                yield event.plain_result("已开启私聊记录。")
            elif val in ("off", "false", "0"):
                self._plugin._record_private_chats = False
                await self._plugin._put_kv_value("record_private_chats", False)
        ...
        *args: str,
        ...
        try:
            ...
            if action is None or action.lower() == "list":
                if not self._plugin._group_whitelist:
                    yield event.plain_result("群白名单为空（记录所有群聊）。")
                else:
                    yield event.plain_result(f"群白名单：{', '.join(self._plugin._group_whitelist)}")
                return
            action_lower = action.lower()
            if action_lower == "clear":
                self._plugin._group_whitelist = []
                await self._plugin._put_kv_value("group_whitelist", [])
            ...
            if action_lower in ("add", "remove"):
                ...
                if action_lower == "add":
                    ...
                    for gid in group_ids:
                        if gid not in self._plugin._group_whitelist:
                            self._plugin._group_whitelist.append(gid)
                            added.append(gid)
                    if added:
                        await self._plugin._put_kv_value("group_whitelist", self._plugin._group_whitelist)
                    ...
                    removed = []
                    for gid in group_ids:
                        if gid in self._plugin._group_whitelist:
                            self._plugin._group_whitelist.remove(gid)
                            removed.append(gid)
                    if removed:
                        await self._plugin._put_kv_value("group_whitelist", self._plugin._group_whitelist)
    ...
    async def record_sanitize_command(
        ...
        try:
            ...
            if denied:
                ...
                return
            if flag is None or str(flag).strip().lower() == "status":
                state = "on" if self._plugin._enable_sanitization else "off"
                yield event.plain_result(f"数据脱敏状态：{state}")
                return
            val = str(flag).strip().lower()
            if val in ("on", "true", "1"):
                self._plugin._enable_sanitization = True
                await self._plugin._put_kv_value("enable_sanitization", True)
                yield event.plain_result("已开启数据脱敏（手机号、token、cookie等将被脱敏）。")
            elif val in ("off", "false", "0"):
                self._plugin._enable_sanitization = False
                await self._plugin._put_kv_value("enable_sanitization", False)
